...
import { dayKeyLabel, deviceAccent, timeAgo } from "../../utils/timeline";
...
const ClickTimeline = ({
  ...
  const showLoadMore =
    isLatest && !isLoading && timeline.length >= limit && timeline.length < totalClicks;
    ...
    >
      ...
      <div className="flex items-center gap-2 overflow-x-auto pb-2 -mx-1 px-1 mb-4 overscroll-contain">
        <button
          ...
          className={`${chipBase} ${isLatest ? chipActive : chipIdle}`}
        >
          Latest
          <span
      ...
      {isLoading ? (
        <div className="flex flex-col gap-2">
          {[1, 2, 3].map((i) => (
            <div
              key={i}
              className="h-16 rounded-lg border border-[#E5E5EA] bg-[#FAFAFA] animate-pulse"
            />
          ))}
        </div>
