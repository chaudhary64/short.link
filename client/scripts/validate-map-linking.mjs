...
const unmapped = world.features.filter(
  (f) => !nameToCode.has(f.properties.name),
);
const unexpected = unmapped.filter((f) => !knownNonISO.has(f.properties.name));
if (unexpected.length) {
  failed = true;
  console.log(
    "UNMAPPED FEATURES (need an alias):",
    unexpected.map((f) => f.properties.name).join(", "),
  );
} else {
  console.log(
...
const codeToName = new Map(
  [...nameToCode.entries()].map(([name, code]) => [code, name]),
);
...
for (const code of nameToCode.values())
...
for (const f of appFeatures) {
  ...
  for (const s of d.match(/[ML][^MZ]*/g) || []) {
    ...
    if (maxX - minX > 180 && maxY - minY < 8) {
      wrapArtifacts++;
      if (wrapArtifacts <= 5)
        console.log(`WRAP ARTIFACT in ${f.properties.name}`);
    }
  }
}
if (wrapArtifacts) {
